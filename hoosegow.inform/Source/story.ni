...
Carry out time-checking:
	...
	begin;
		...
		otherwise say "Your pocket watch reads [time of day + 1 minute].";
	otherwise;
...
"town" or "crawdad's gulch" or "the town"	"A more inhospital piece of dirt west of the Mississippi I don't know"		"Yessiree. This here city got almost five hundred people and even some indoor plumbing some of the time"
...
To say cup-muddy:
	say "I reckon coffee is the fourth best thing in the world".
...
To say harmonica-deputy:
	say "It were an unfortunate day when them blue-haired old biddies done stuck that harmonica in the jail cell. I hate them jailhouse blues".
...
This is the blowing rule:
	...
	otherwise:
		...
		if the gray bar does not contain the berry:
			say "You might use your tube as a blowgun, but it's got an emptiness to it right now.";
			the rule fails.
	...
	if the player does not carry the gun:
		...
		otherwise:
			if the player can see the gun:
				...
				now the player carries the gun;
			otherwise:
				say "You'd need a gun to shoot off anything besides your mouth. And even for that, having a gun nearby is recommended." instead.

Carry out shooting:
	change the block stage business flag to true;
	say "You squeeze the trigger (click). [one of]Dang, no bullets. [or][stopping][paragraph break]";
	if the sheriff is in the jailhouse:
		say "A barrage of pistol discharges cuts through the air. You try to imagine that the bullets are just insubstantial phantoms, but that doesn't pan out for you in the end. Riddled with bullets and torn to pieces, your body collapses in a bloody mess.";
...
Instead of showing something (called the thingie) to a person (called the observer):
	if the observer is:
		...
		-- the deputy:
			...
			otherwise:
				say "The deputy seems [one of]annoyed[or]irritated[or]put out[or]irked[or]bored[or]like he's in a bad mood[or]like he has better things to do[at random], but looks [if the thingie is not in the location of the deputy]over [end if]at [the thingie] and says, [quotation mark]";
				if the thingie is:
					-- pocketwatch:
						say "[watch-deputy]";
					-- spoon:
					...
					-- feather:
						say "[feather-deputy]";
					-- otherwise:
						say "[one of]Uh huh[or]Yep. Real nice. Now leave me alone[or]Humph[at random]";
				say ".[quotation mark][paragraph break]";		
		-- Flash:
...
Carry out directedSpitting at:
	...
	if the noun is:
		...
		-- Flash: 
			...
			otherwise:
				say "The vision of what happened last time is seared into your brain like a brand on the rump of a prize cow. You do not want to see that again." instead;
		-- deputy: 
...
0					"starts to rub a flea but then decides it to be too much work"
...
This is the Pete's stage business rule:
	...
	if Pete is not in Limbo and a random chance of 4 in 20 succeeds:
		if a random chance of 4 in 20 succeeds:
			say "[the pete] [one of]jumps straight up, immediately animate[or]springs off the bench[or]spins in the air[or]leaps into the middle of the jail cell[or]throws his head back, howls, and stands on the bench[or]somersaults from the bench into the center of the jail cell[or]climbs up the jail bars, his arms swinging wildly[or]bounces from wall to wall[at random]. ";
			...
			say "![quotation mark][paragraph break][the pete] [one of]collapses[or]dives[or]crumples[or]faints[or]falls[at random] onto the bench and [one of]falls back to sleep immediately[or]is soon lost in dreams again[or]resumes snoring without missing a beat[or]resumes his previous position as if nothing had happened[at random].";
		otherwise:
			say "[the pete] ";
...
0		"Në gjuhët! U movakh! V ezitsite! En les llengües! Zài shé tou! U yezika! V jazyky! I tunger! In de tongen! In keeled! Sa wika! Kun kielet! Dans les langues! Nas linguas! In den Zungen! Stin glosses! Jihba mem! In die tonge! A nyelv! Í tungum! Dalam bahasa! I tongues! Nelle lingue! Shita de! Bang-eon! In mēlītes! Be kalbų! Vo Jazicite! Fil-ilsna! W języki! Nas línguas! În limba! V yazikakh! In jezikov! En las lenguas! Katika lugha! I tungor! Nı lîn! Yn y tafodau! I tungor, bork, bork, bork.[quotation mark][paragraph break]Muddy,[quotation mark] you ask, [quotation mark]What do you suppose he's on about?[quotation mark][paragraph break][quotation mark]Muddy replies sagely, [quotation mark]Speaking in tongues, I reckon"
0		"There shall be an ACCOUNTING and it shall come swiftly, and with little notice, most likely in a four hour time slot in the morning, but perhaps in the late afternoon or evening if they're running late. THE WAGES OF SIN shall be entered into the ledgers, in duplicate, after first deducting the withholdables of sin and carrying over any offsetting credits from previous GOOD WORKS, and taking into consideration the INFLATION of EVIL, the DEVALUATION of MORALS and the exchange rate with PAGAN BELIEFS"
...
0					"pedals the air with his feet, while making sounds like a ringing bell."
0					"rolls back and forth on the bench."
0					"curls up into a ball, muttering to himself."
0					"froths at the mouth."
0					"stares wide-eyed at the ceiling for a moment and then falls back asleep."
...
Instead of giving the warrant to Muddy:
	...
	if the ask-me counter is not 6:
		say "Muddy brushes it aside and says, [quotation mark][one of]Wait a minute. I ain't told you my plan yet. Don't go all a-jumping in front of my wagon. Why don't you ask me about the plan[or]Could you ask me my plan first[stopping]?[quotation mark][paragraph break]";
		the rule fails;
...
The telegram is a sheet in the folder. The description of the telegram is "A yellow paper that you recognize as the stationery of the Western Union telegraph company. The message on the telegram is addressed to the sheriff and reads:[telegram-text]." Understand "paper", "yellow", and "message" as the telegram. The texture of the telegram is "like cheap, almost tissue-thin paper".
...
The floor is a backdrop in the jail cell. The description of the floor is "Rough, dirty[if the stain is on the floor], and stained[end if] concrete." The scent of the floor is "remarkably like every other jailhouse floor you have ever taken the time to examine closely". The texture of the floor is "gritty".

Section Gate

The gate is a large door.  The gate is scenery.  The gate is west of the office and east of the Jail Cell.  The gate is lockable and locked.  The description of the gate is "[if the gate is locked]A metal gate stands between you and freedom. [end if]The gate is set into the metal bars that surround your cell, and its hinges must be internal. The gate has a massive padlock [if the gate is locked]that clicked definitively behind you when you were thrown into the cell[otherwise]of great antiquity[end if]. You've seen banks with poorer locks. At least this is a quality institution." The texture of the gate is "cold and unyielding".
...
The bench is a large furniture in the jail cell.  The description of the bench is "A long wooden bench made of rough, splintery planks[if pete is on the bench]. You ignore the man with the black suit who is lying on the bench[end if][if the bench is not investigated]. [bench sekrits][end if]." The bench can be investigated. The bench is not investigated. The texture of the bench is "like rough wood".
...
Instead of inserting something (called the item) into the gap:
	if the item is the gray bar:
		say "The gray bar could fit back in the gap, but it won't stay attached.  Not much point in that.";
	otherwise:
...
The vine is scenery in the jail cell.  The description of the vine is "Only the leafless tip of a wiry vine pokes into the cell through the window.[if the player is tall] It's covered with small red berries.[end if]".

Instead of taking the vine:
	say "You yank on the wiry vine and it yanks back. It's too tough to break off. From outside the window, you hear the bulk of the plant rustling as it swings back and forth."
	
Instead of pulling the vine:
	try taking the vine.
	
Instead of pushing the vine:
	say "About as effective as pushing a string."
...
To warn about the (cylinder - a thing):
	...
	if the angerometer is:
		...
		-- 3:
			say "The deputy rubs his head with his palms and growls, [quotation mark]I'm a-warning you first instead of a-shooting you first because I got myself a bad headache and I don't need the extra noise. Keep messing with them [if the cylinder is the bars]jail bars[otherwise]window rods[end if] and I will scratch my itchy trigger finger.[quotation mark][paragraph break]Muddy raises his eyebrows at you to get your attention and whispers, [quotation mark]He ain't kidding, Rick. We best not push him too far. He's a rattlesnake in a bottle of tequila.[quotation mark][paragraph break][quotation mark]What the blazes does that mean, Muddy?[quotation mark][paragraph break][quotation mark]Just metonymy, I reckon.[quotation mark][paragraph break]";
		-- 4:
...
Instead of singing when the deputy is singing-sensitive:
	say "The deputy sighs, raises his gun and fires. You catch the bullet smack dab in the forehead.[paragraph break]As the lights fade, Muddy bends over you and closes your eyes, whispering, [quotation mark]I ain't gonna give you up, I ain't gonna let you down...[quotation mark][paragraph break]";
	change the block stage business flag to true;
...
Instead of searching the deputy when the deputy is not awake:
	if the deputy carries the warrant:
		say "You rifle through his trouser pockets and find a federal warrant. You also find a small brass key. Naturally, you take both.";
...
Flash is a male animal in Limbo. Flash is proper-named. The description of Flash is "A mangy old mutt well past his prime." Flash can be spat upon. Flash is not spat upon. Understand "dog" or "pooch" or "canine" or "bloodhound" or "mutt" as Flash.
...
Before giving something (called the item) to Muddy:
	if the player does not enclose the item:
		say "You ain't got [the item]. How's a body to give something away what they ain't got a hold of anyhow?";
		stop the action.
...
Instead of inserting something (called the item) into the pocket (this is the block putting stuff into Pete's pocket rule):
	...
	otherwise:
		say "Muddy catches your hand before you can put [the item] into [the pete]'s pocket. He chides you, [quotation mark]Taking stuff out of people's pockets is one thing, Rick, but you ain't got no business putting stuff in. It's a violation of a person's privacy I do reckon."
...
The pamphlet is a sheet in the pocket. The description of the pamphlet is "The pamphlet depicts Zeus in a cowboy hat roasting sinners over a campfire. A sermon is printed below the picture." The texture of the pamphlet is "like cheap newsprint". Understand "sermon" as the pamphlet. The inscription of the pamphlet is "[one of]You read it aloud:[paragraph break][pamphlet sermon][paragraph break][initial pamphlet][or][second pamphlet][or][pamphlet sermon][stopping]". 
...
Instead of searching the pocket:
	...
	otherwise:
		say "You don't find anything that [the pete] wasn't born with."
		
Before taking the tin:
	if the tin is in the pocket:
		say "You borrow [the pete]'s tin of chewing tobacco. It don't look like he'll mind, seeing as how he's unconscious and all.";
		move the tin to the player;
		stop the action.

The description of the tin is "A rectangular tin of [quotation mark]Spitting Image[quotation mark] chewing tobacco. The cover shows a cowboy painting a portrait of an Indian, and true to its name, the image on the canvas looks just like the Indian. The tin is [if the tin is open]open[otherwise]closed[end if][if the tin is open] revealing some [tobacco-appearance][end if]."
...
To say hot diggity dog:
	say "Muddy grabs it from you, squinting and sounding out words silently with his mouth. His eyes open wide in amazement.[paragraph break][quotation mark]Pete? Wait a gosh darn... PASTOR Pete? That really him? Hot diggity![quotation mark][paragraph break]Muddy jumps to his feet, runs to the bench and shakes Pete wildly; Pete screams and throws his hands into the air, gesticulating to heaven as he's awoken from his dream --[paragraph break][quotation mark]BEGONE, daemons! The Prince of Piety shall SMITE thee! He who brushes the Celestial Cavalier's horses shall not clean the stables of PARADISE![quotation mark][paragraph break]The grubby preacher collapses back on the bench, eyes closed and snoring before either of you can take a step."
...
To say show-telegram:
	say "You show the telegram to Marshal McLuhan, telling him that [if silver-found is false]he'll find the sheriff's[otherwise]the sheriff had instructed his men to hide the[end if] stagecoach full of stolen silver just behind the jail.[paragraph break][if silver-found is false][forgot-coach][end if]".
...
To say need-more-evidence-than-receipt:
	say "The marshal looks disappointed and says, [quotation mark]Being in debt is no crime, although it does eat away at the soul. Once, I went an entire week owing more than five dollars, and I regret it to this day. However, there is nothing to suggest that the sheriff [if silver-found is true]had any sort of plan that would have required this much money[otherwise]has resorted to any illegal measures to fund his plans[end if].[quotation mark][paragraph break]".
...
To say show-patent:
	say "You show the patent to the marshal and intimate that the sheriff had extensive business plans, far beyond his mandate as a public servant. [paragraph break]"
...
To say whistled-to-death:
	say "The deputy stumbles in from the street, barely able to stand. Regarding you through bloodshot and unfocused eyes, he whines, [quotation mark]I done told you enough times not to set off that whistle and did you listen? Heck, no. Well, maybe this will teach you a lesson![quotation mark][paragraph break]He staggers forward and draws his gun. The first shot goes into the pipe above the steam boiler, puncturing the pipe and sending a jet of hot steam into the Deputy's face. The deputy howls in pain and shoots again, hitting the dinner bell. Flash darts forward, tripping the deputy and sending him sprawling on the floor. The deputy curses and fires a volley of three more shots into the ceiling, which showers him with concrete fragments, blinding him. The next shot goes wide, straight out the jail cell window, and brings down a hawk that had been circling overhead. With his final shot, the deputy shoots the floor just in front of his left boot.[paragraph break]By some quirk of physics and happenstance, the exact nature of which is debated to this very day, the bullet ricochets off the wooden cabinet, clips the ends of the portrait frame, rebounds off the bricks above [the pete]'s bench, vibrates rapidly between the jail bars, kicks the bean can into the air, and flies in a final straight line that includes both your head and Muddy's."
...
Instead of taking the feather:
	if the feather is part of the tail:
		say "You pluck a feather from the vulture's tail. The bird twists its head momentarily to glare at you before plunging back into its task of devouring the rotting meat.";
		adjust points by one;
...
To say plan2-askme:
	say "[one of]Muddy gives you that familiar old look that always leads to trouble. [quotation mark]I got me another cunning plan. C'mon, ask your old buddy Muddy...[quotation mark] Muddy leans forward, clearly impatient for you to ask him about his latest stroke of (in his opinion) genius. [or][quotation mark]No, really. This plan is a winner. I'm dying for you to ask me about it. You'll see. This one will work.[quotation mark] Muddy looks desperate to prove himself to you. [or][quotation mark]I know some of my plans haven't panned out entirely, but I see a way clear out of this jail. Go on, ask me.[quotation mark] It's like the years have caught up to Muddy, and he wants this one more chance to do right by you. [or][quotation mark]Rick, will you trust me on this one? Ask me my plan.[quotation mark] Muddy looks at the ground, despondent that you are ignoring him. [stopping]".
	
To say plan2-blurt:
	say "Muddy fidgets. No longer able to contain himself, he blurts out, [quotation mark]Oh for the sake of criminy, I got to tell you![quotation mark] "
...
"Stuff you do a lot"		--	"We made it easy to do some stuff without typing a whole lot. You can use the letter in [fixed letter spacing]<brackets>[variable letter spacing] instead of having to spell out the whole word, which is good on account some folk don't spell so good (not to mention their grandma and sin tax):[paragraph break]
...
"Reading & Writing"		--	"If'n you got room in yer saddle bag, you can save games and then reload them later. You can make multiple save files. You can also restart a game (but it'll beef your current game). If you're desperate, you can turn tail and cut.[paragraph break]
...
LibMsg <undo failed>			"[apostrophe]Undo[apostrophe] failed real miserable-like. [bracket]Not all interpreters got the cojones to get undid.[close bracket][paragraph break]"
...
LibMsg <cannot exceed carrying capacity>	"You're carrying too dang-blasted many things already.[paragraph break]"    
...
LibMsg <pronoun not set>		"I don't rightly twig what who you're talking about.[paragraph break]"  
...
"[if the player wears the right boot and the player wears the left boot]You have been told on occasion by some not very tactful acquaintances of yours that your feet have a distinct scent[otherwise]If one boot is bad, imagine two[end if]."
...
"Does the gray bar have any special qualities? Maybe something that might explain why it vibrated differently than the other bars?"
...
"You can BLOW the berry AT the coffee."
...
hint												used
"How clever. You've doped the deputy's coffee, so if he drinks some, it will put him to sleep instead of waking him up. Now the question is -- how do you get him back to the jailhouse?"
...
"Taking off your boots is sure to make Flash want to give you some distance."
...
"The lever looks like it could swing either towards or away from you."
...
"Flash can move the lever by walking either towards or away from the jail cell."
...
"How did the deputy let Flash know he had some food for him?"
...
hint												used
"Now that the big oaf has collapsed next to the jail cell, you can reach him."
"He's even less talkative than usual in this state, so witty repartée is probably not going to get you very far."
...
"You need to find something in the [if the drawer is not rewarded]office[otherwise]drawer[end if] to take the marshal's mind off of hanging you and Muddy."
...
"Show the marshal the telegram, the receipt, and the patent."
...
At the time when Muddy comments about departure:
	...
	if the deputy is not harmonicated:
		if the can of beans is consumed:
			say "[quotation mark]For you maybe,[quotation mark] you reply. [quotation mark]But I near to lost my lunch what with that stench.[quotation mark][paragraph break]To which Muddy counters, [quotation mark]Ain't much worse than your feet, I reckon.[quotation mark][paragraph break]";
		otherwise:
...
This is the flashing stink rule:
	if flash is in the office and flashing is happening:
		...
		if the ambient odor is 100:[for any reason]
			say repel the dog;
			if the number of bootlike things that are worn is zero:
				say "You agree with Muddy that the deputy is not likely to be in a good mood when he arrives, so you put your boots back on.";
				now rick wears the left boot;
				now rick wears the right boot;
...
Instead of going a direction (called the way) during the PlusQueDenouement:
	if the way is east:
		if the player is in the office:	
			say "[one of]Muddy relents and says, [quotation mark]All right, Rick. I reckon we done what we can do. Let's make for the hills.[quotation mark][paragraph break]As you head out of the office, two men appear in the doorway: Sheriff Cheney and Federal Marshal McLuhan. They are followed by a small detachment of U.S. Army guards[or]The army guards block your exit[stopping].";
	...
	if the way is west:
		if the player is in the office:
			continue the action;
		otherwise:
			say "You can't go any farther west than the jail cell.";
			the rule succeeds;
	otherwise:
...
Instead of showing something (called the evidence) to someone (called the spectator) during the plusquedenouement:
	if the spectator is:
		...
		-- the marshal:
			if the evidence is:
				-- the warrant:
					say "The marshal examines the warrant briefly and says, [quotation mark]This warrant is real, although that last ludicrous bit is obviously forged. How could you ever hope to find anyone stupid enough to fall for such an obvious trick?[quotation mark][paragraph break]";
					say "[death-note]";
					change the endgame to hanged;
					end the game in death;
				-- the note:
...
say "* In the real Old West, it wouldn't have made any sense to put beans in a can. They were a cheap staple, transported dry, likely in sacks.[paragraph break]";
say "* Boot spurs were referred to as [quotation mark]can openers[quotation mark] in the Old West. Really.[paragraph break]";
say "* The transcontinental railroad was completed in 1869, but it crossed two states to the north of Oklahoma. The first railroads in Oklahoma were built in 1870, and the state wasn't crossed until 1871.[paragraph break]";
